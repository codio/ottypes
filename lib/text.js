  ...
  for (var i = 0; i < op.length; i++) {
    ...
    switch (typeof component) {
      ...
      case 'string':
        ...
        break;
      case 'object':
        addToInvertArray(lastPos, component.d, str.substring(0, component.d));
...
var invertDeleteOpArray = [];
var invertDeleteOpArrayMaxLen = 10;
var addToInvertArray = function(pos, count, str){
    //save only invertDeleteOpArrayMaxLen delete operations
    var len = invertDeleteOpArray.length;
    if (len > invertDeleteOpArrayMaxLen){
        invertDeleteOpArray = invertDeleteOpArray.slice(len - invertDeleteOpArrayMaxLen, len );
    }
    invertDeleteOpArray.push( { hash: pos + ':' + count, str : str} );
};
var findInInvertArray = function(pos, count){
    var hash = pos + ':' + count;
    for (var i = invertDeleteOpArray.length - 1; i >=0; i--){
        if (invertDeleteOpArray[i].hash == hash){
            var str = invertDeleteOpArray[i].str;
            invertDeleteOpArray.splice(i, 1);
            return str;
        }
    }
    return '';
...
exports.invert = function(op) {
    ...
    for (var i = 1; i < op.length; i+=2) {
        if (op[i] instanceof Object){
            op[i] = findInInvertArray(op[i - 1], op[i].d);
        } else {
            op[i] = {d:op[i].length};
        }
    }
